...
class BoundedIPAccessTracker:
    def __init__(self, max_ips_per_channel=1000, max_channels_per_ip=10, timeout=8):
            ...
            try:
                with self.lock:
                    ...
                    if self.channel_manager:
                        for channel_name in list(self.channel_ips.keys()):
                            if not self.is_active(channel_name):
                                ch = self.channel_manager.channels.get(channel_name)
                                if ch and ch.state in [ch.STATE_RUNNING, ch.STATE_STARTING]:
                                    print(f"强制停止空闲频道: {channel_name}")
                                    self._force_stop_channel(channel_name)
            ...
            while True:
                ...
                try:
                    with self.lock:
                        for name, ch in self.channels.items():
                            if self.ip_tracker:
                                is_active = self.ip_tracker.is_active(name)
                                if not is_active and ch.state in [ch.STATE_RUNNING, ch.STATE_STARTING]:
                                    print(f"监控线程: 停止不活跃频道 {name}")
                                    self._force_stop_channel(name)
                except Exception as e:
                    pass
...
def serve_hls(channel_path):
    try:
        ...
        if filename.endswith(".m3u8"):
            ...
            if not os.path.exists(file_path):
                return jsonify({"error": "Stream file sync error"}), 500
            
            response = _serve_file_directly(file_path, "m3u8")
            
            @response.call_on_close
            def on_response_close():
                if manager.ip_tracker:
                    manager.ip_tracker.remove_ip(client_ip, channel_name)
                    print(f"客户端 {client_ip} 断开连接，频道 {channel_name}")
            
            return response
        
        elif filename.endswith(".ts"):
            if not os.path.exists(file_path):
                return jsonify({"error": "TS文件不存在"}), 404
    ...
    with manager.lock:
        for name, other_ch in manager.channels.items():
            if name != channel_name and other_ch.state in [other_ch.STATE_RUNNING, other_ch.STATE_STARTING]:
                if manager.ip_tracker and not manager.ip_tracker.is_active(name):
                    print(f"换频道：停止没有活跃客户端的旧频道 {name}")
